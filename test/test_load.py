...
def test_load ():
  with StringIO(TEST_CODE) as stream:
    loaded = robotstxt.load(stream)
...
def test_load_error ():
  with StringIO(TEST_CODE_ERROR) as stream:
    with pytest.raises(robotstxt.ParseError):
      loaded = robotstxt.load(stream, ignore_error=False)

def test_load_ignore_error ():
  with StringIO(TEST_CODE_ERROR) as stream:
    loaded = robotstxt.load(stream, ignore_error=True)
    assert len(loaded.permissions) == 4
    assert "*" in loaded.permissions
    assert loaded.permissions["*"].allows == set()
    assert loaded.permissions["*"].disallows == {"/"}
